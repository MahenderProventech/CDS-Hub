...
const DataTable = () => {
    ...
    useEffect(() => {
       console.log(userData);
        ...
        http.get(`ExecutionFormula/GetExecutionDashboard?empoyeeid=${userData.employeeId}`)
            .then((resp) => {
                ...
                let gridData = rawData.filter(each => each.formulaId); // Filtering data
                ...
                <nav aria-label="breadcrumb" className="mainBreadcrumb ">
                    <div className="position-relative">
                        <ol className="breadcrumb p-0" style={{fontSize:'12px',background:'white'}}>
                            <li className="breadcrumb-item"><Link to={"/home/master-dashboard"}>Execution Dashboard</Link></li>
                        </ol>
                                ...
                                <div className="table-responsive-x">
                                    <table className="table table-text-small mb-0">
                                        <thead className="thead table-sorting">
                                            <tr>
                                                <th>#</th>
                                                <th width="250px" onClick={() => handleSort('formulaId')}>Formula Id</th>
                                                <th width="250px" onClick={() => handleSort('description')}>Description</th>
                                        ...
                                        <tbody>
                                            {getData(current, size).map((data, index) => (
                                             ...
                                             <Link to={`/home/execApproveFormulaByID/${data.id}`}>{data.formulaId}</Link>
                                              ) : (
