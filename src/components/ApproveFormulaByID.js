...
const ApproveFormulaByID = () => {
  ...
  const [testName, setLabelTestName] = useState('');
  const [groupName, setLabelGroupName] = useState('');
  ...
  const showAlert = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        navigate("/home/reviewApproveDashboard")
  ...
  const showChildAlert = (callback) => {
    ...
    <textarea id="swal-comments" class="swal2-input" placeholder="Comments" style="padding: 5px; width: 90%; margin-top: 10px; font-size: 12px; height: 80px; border-radius: 4px; border: 1px solid #ccc;"></textarea>
      ...
      preConfirm: () => {
        ...
        const comments = Swal.getPopup().querySelector('#swal-comments').value;
      ...
      if (result.isConfirmed) {
        const { username, password, comments } = result.value;
  ...
  const saveData = async () => {
    try {
      showChildAlert(async (comments) => {   
        ...
        const resp = await http.post("Formulas/UpdateFormulaStatus", obj);
  ...
  const saveRejectFormula = async () => {
    try {
      showChildAlert(async (comments) => {  // Accept comments in the callback
        const obj = {
          Status: "Rejected",
          Id: params.id,
            ...
            <ul className="inner">
              ...
              <div style={{ background: "#fff", padding: "2rem 2rem 0rem" }}>
                  ...
                  <div className="col text-end mt-4 mb-5">
                    <button
                      className='btn btn-primary' onClick={saveData} style={{ marginRight: "15px" }}
                    >
                      Approve
                    </button>
                    <button
                      className='btn btn-primary' onClick={saveRejectFormula}
                    >
                      Reject
                    </button>
                  </div>
...
export default ApproveFormulaByID;
